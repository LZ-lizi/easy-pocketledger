"""Turn the normalised SVGs into the VectorDrawables the app actually ships.

`svg/` holds hand-made artwork and is read only by `normalize_icons.py`. This reads the
result of that pass, `normalized/`, which is already on the app's 100 x 100 canvas at one
size and one stroke weight.

VectorDrawable's `pathData` accepts the same syntax as SVG -- including the arcs the drawn
icons are made of -- so a `<path>` transfers verbatim and cannot drift from what the
preview showed. Only the three things VectorDrawable cannot express have to be translated:
...
SVG_DIR = os.path.join(HERE, "normalized")
...
# The launcher icons stay plain VectorDrawables in the project and are not part of this set.
...
def parse_children(element, inherited, paths):
    ...
    for child in element:
        ...
        if tag == "g":
            # A bare `<g>` is only a container; a transformed one matches the single form
            # the normalised artwork uses.
            match = re.match(
                r"translate\(([-\d.]+),([-\d.]+)\)\s*scale\(([-\d.]+)\)",
                child.get("transform", ""),
            )
            if child.get("transform") and not match:
                raise SystemExit(f"unsupported <g transform>: {child.get('transform')}")
            if match:
                paths.append(("group", tuple(float(v) for v in match.groups())))
            parse_children(child, inherited, paths)
            if match:
...
def document(name, paths):
    ...
    return (
        ...
        "<!-- Generated by icons/make_drawables.py from icons/normalized/"
        f"{name}.svg. Edit the artwork, not this file. -->\n"
