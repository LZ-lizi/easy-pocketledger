...
data class ImportRow(
    ...
    /** 1-based line in the source file, so a bad row can be pointed at. */
    val lineNumber: Int,
) {
    /**
...
/**
 * Reads WeChat, Alipay and generic CSV bill files.
 *
 ...
 *
 * Three things about these files drive the design:
 *
...
object CsvImport {
    ...
    /** Reads a whole file into importable rows, reporting what it had to drop. */
    fun parse(text: String): ImportPreview {
        val table = readTable(decodeBom(text))
        val format = detect(table)
        val headerIndex = table.indexOfFirst { isHeaderRow(it) }
        if (headerIndex < 0) {
            return ImportPreview(format, emptyList(), listOf("没有找到表头行"), 0)
        }
        ...
        for (line in headerIndex + 1 until table.size) {
            ...
            val typeText = columns.text(fields, TYPE_NAMES).ifBlank { "" }
            val normalisedType = normalise(typeText)
            if (normalisedType in NON_TRANSACTION_TYPES) {
            ...
            val type = when {
                ...
                normalisedType.isEmpty() -> TxnType.EXPENSE
                else -> null
            }
            if (type == null) {
            ...
            val stamp = columns.text(fields, DATETIME_NAMES).ifBlank { null }
            val datePart = stamp?.substringBefore(' ')
            val dateKey = datePart
                ?.let(::parseDateKey)
                ?: columns.text(fields, DATE_NAMES).let(::parseDateKey)
            if (dateKey == null) {
                drop("日期无法识别：${stamp ?: columns.text(fields, DATE_NAMES).ifBlank { "空" }}")
                continue
            }

            val time = stamp?.let(::parseTime)
                ?: parseTime(columns.text(fields, TIME_NAMES))

            val merchant = columns.text(fields, MERCHANT_NAMES).ifBlank { null }
            val note = columns.text(fields, NOTE_NAMES).ifBlank { null }
            rows.add(
                ImportRow(
                    dateKey = dateKey,
                    time = time,
        ...
        return ImportPreview(format, rows, reasons, skipped)
    }
    ...
    private val MERCHANT_NAMES = listOf("交易对方", "商家", "对方", "商户名称")
    private val NOTE_NAMES = listOf("商品说明", "商品", "备注", "分类", "交易分类")
    ...
    /** `¥1,240.50` and `1240.50元` both have to become `1240.50`. */
